<?php
    ...
    class Resultados extends CI_Controller {
        ...
        public function generaReporteUAED(){


            $arrCab=$this->rc_model->dameCabeceraNoCheck();
            // echo "<pre>".print_r($arrCab,false)."</pre>";die();
            ...
            foreach ($arrCab as $cab_key => $cab_value) {
                ...
                $cabecera['ua']=$arrCabecera['unidad'];
        ...
        public function generaReporteDocED(){

            $arrCab=$this->rc_model->dameCabeceraNoCheckDoc();
            // echo "<pre>".print_r($arrCab,false)."</pre>";die();

            foreach ($arrCab as $cab_key => $cab_value) {
                $dni_cabecera=$cab_value['dni']; 
                $id_tipo=1; // tipo de encuesta (1)docente (2)asignatura (3)infraestructura
                
                $arrCabecera=$this->ev_model->dameCabeceraPorDNI($dni_cabecera);

                $cabecera['periodo']=$arrCabecera['periodo'];
                $cabecera['docente']=$arrCabecera['docentes'];
                $cabecera['dni']=$arrCabecera['dni'];
                $cabecera['unidad']=$arrCabecera['unidad'];

                echo "<pre>".print_r($cabecera,false)."</pre>";
